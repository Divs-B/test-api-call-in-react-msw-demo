...
const filmsDataTask = rest.get("https://ghibliapi.herokuapp.com/films", async (req, res, ctx) => {
  const films = req.body;
  res(ctx.json(films))
});


export const filmError500Task = rest.get("https://ghibliapi.herokuapp.com/films", async (req, res, ctx) => {
  return res(ctx.status(500), ctx.json({ message: "Oops… something went wrong, try again 🤕" }))
});

export const filmError418Task = rest.get("https://ghibliapi.herokuapp.com/films", async (req, res, ctx) => {
  return res(ctx.status(418), ctx.json({ message: "418:I'm a tea pot 🫖, silly" }))
});


export const handlers = [filmsDataTask];
